...
with glimmung-internal bookkeeping (`issue_lock_holder_id`, etc.).
GitHub's workflow_dispatch endpoint 422s any input the workflow file
doesn't declare, so leaking internal fields produces a silent failure:
the dispatch raises, the exception is logged-not-surfaced, the Run
lives on as IN_PROGRESS with no workflow_run_id forever.
