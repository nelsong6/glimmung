...
def test_workflow_path():
    assert (
        paths.workflow_path("ambience", "agent-run")
        == "projects/ambience/workflows/agent-run"
    )
...
def test_attempt_path():
    assert (
        paths.attempt_path("ambience", "01KR23", 1)
        == "projects/ambience/runs/01KR23/attempts/1"
    )
...
def test_path_for_project():
    p = Project(id="ambience", name="ambience", github_repo="nelsong6/ambience",
...
def test_path_for_workflow_uses_project_field():
    ...
    w = Workflow(
        ...
        phases=[], budget=BudgetConfig(total=25.0),
        created_at=datetime.now(UTC),
    )
...
def test_path_for_run_uses_run_fields():
    ...
    run = Run(
        id="01KR23", project="ambience", workflow="agent-run",
        run_number=1, issue_number=172,
        attempts=[], cumulative_cost_usd=0.0,
...
def test_path_for_attempt_and_job_and_step():
    ...
    job = NativeJobAttempt(
        job_id="agent-execute", name="run agent and verify result",
        steps=[step],
    )
    attempt = PhaseAttempt(
        attempt_index=1, phase="agent-execute",
        workflow_filename="k8s_job:agent-execute",
        dispatched_at=now, jobs=[job],
    )
    run = Run(
        id="01KR23", project="ambience", workflow="agent-run",
        run_number=1, issue_number=172,
        attempts=[attempt], cumulative_cost_usd=0.0,
        budget=BudgetConfig(total=25.0),
        state=RunState.IN_PROGRESS,
        created_at=now, updated_at=now,
    )
    assert paths.path_for_attempt(run, attempt) == (
        "projects/ambience/runs/01KR23/attempts/1"
    )
