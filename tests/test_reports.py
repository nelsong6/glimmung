...
`find_report_by_repo_number` helper used by the webhook mirror, and the
`append_report_comment` / `append_report_review` primitives the consumer PR will
...
from glimmung.models import ReportComment, ReportReview, ReportReviewState, ReportState
from glimmung.reports import (
    append_report_comment,
    append_report_review,
    close_report,
    create_report,
    ensure_report_for_github,
    find_report_by_repo_number,
...
async def test_create_pr_persists_with_open_state_and_defaults(cosmos):
    ...
    result = await read_report(cosmos, project="ambience", report_id=pr.id)
...
async def test_update_pr_patches_specified_fields_only(cosmos):
    ...
    fetched, etag = await read_report(cosmos, project="ambience", report_id=pr.id)

    updated, new_etag = await update_report(
...
async def test_update_pr_recovers_from_stale_etag_via_retry(cosmos):
    ...
    fetched, stale_etag = await read_report(cosmos, project="ambience", report_id=pr.id)
    await update_report(cosmos, pr=fetched, etag=stale_etag, title="winner")

    second, _ = await update_report(cosmos, pr=fetched, etag=stale_etag, title="second")
...
async def test_close_pr_transitions_state_without_merge(cosmos):
    pr = await create_report(cosmos, project="a", repo="r/n", number=1, title="t", branch="b")
    fetched, etag = await read_report(cosmos, project="a", report_id=pr.id)
    closed, _ = await close_report(cosmos, pr=fetched, etag=etag)
    assert closed.state == ReportState.CLOSED
...
async def test_close_pr_is_idempotent(cosmos):
    pr = await create_report(cosmos, project="a", repo="r/n", number=1, title="t", branch="b")
    fetched, etag = await read_report(cosmos, project="a", report_id=pr.id)
    closed_once, etag1 = await close_report(cosmos, pr=fetched, etag=etag)
    closed_twice, _ = await close_report(cosmos, pr=closed_once, etag=etag1)
    assert closed_twice.state == ReportState.CLOSED
...
async def test_merge_pr_stamps_merged_at_and_merged_by(cosmos):
    ...
    merged, _ = await merge_report(
        cosmos, pr=fetched, etag=etag,
        merged_by="claude[bot]",
        merged_at=merge_time,
    )
    assert merged.state == ReportState.MERGED
...
async def test_merge_pr_preserves_existing_merge_metadata_on_redelivery(cosmos):
    ...
    assert merged_second.state == ReportState.MERGED
...
async def test_reopen_pr_transitions_back_to_open(cosmos):
    pr = await create_report(cosmos, project="a", repo="r/n", number=1, title="t", branch="b")
    fetched, etag = await read_report(cosmos, project="a", report_id=pr.id)
    closed, etag = await close_report(cosmos, pr=fetched, etag=etag)
    reopened, _ = await reopen_report(cosmos, pr=closed, etag=etag)
    assert reopened.state == ReportState.READY
...
async def test_list_prs_includes_closed_prs(cosmos):
    ...
    fetched, etag = await read_report(cosmos, project="a", report_id=a.id)
    await close_report(cosmos, pr=fetched, etag=etag)

    prs = await list_reports(cosmos, project="a")
    assert sorted(p.id for p in prs) == sorted([a.id, b.id])
    assert {p.id: p.state for p in prs}[a.id] == ReportState.CLOSED
...
async def test_list_open_prs_filters_by_state(cosmos):
    ...
    open_prs = await list_active_reports(cosmos, project="a")
...
async def test_list_open_prs_filters_by_project(cosmos):
    a = await create_report(cosmos, project="a", repo="r/n", number=1, title="a", branch="b1")
    await create_report(cosmos, project="other", repo="r/n", number=2, title="o", branch="b2")
    open_a = await list_active_reports(cosmos, project="a")
...
async def test_list_open_prs_no_project_returns_all_open(cosmos):
    a = await create_report(cosmos, project="a", repo="r/n", number=1, title="a", branch="b1")
    b = await create_report(cosmos, project="b", repo="r/n", number=2, title="b", branch="b2")
    open_all = await list_active_reports(cosmos)
...
async def test_find_pr_by_repo_number_returns_matching(cosmos):
    ...
    result = await find_report_by_repo_number(cosmos, repo="nelsong6/ambience", number=42)
    assert result is not None
    pr, etag = result
...
async def test_find_pr_by_repo_number_crosses_partitions(cosmos):
    ...
    result = await find_report_by_repo_number(cosmos, repo="nelsong6/kill-me", number=7)
    assert result is not None
    pr, _ = result
    assert pr.id == expected.id
...
async def test_find_pr_by_repo_number_returns_none_for_missing(cosmos):
    await create_report(cosmos, project="a", repo="r/n", number=1, title="t", branch="b")
    result = await find_report_by_repo_number(cosmos, repo="r/n", number=9999)
...
async def test_ensure_pr_for_github_creates_when_missing(cosmos):
    pr, etag, created = await ensure_report_for_github(
        cosmos, project="ambience",
        repo="nelsong6/ambience", number=42,
        title="impl",
        branch="claude/fix",
...
async def test_ensure_pr_for_github_returns_existing(cosmos):
    ...
    across redeliveries."""
    first, _, created_first = await ensure_report_for_github(
        cosmos, project="ambience",
        repo="nelsong6/ambience", number=42,
        title="impl", branch="b",
    )
    ...
    second, etag, created_second = await ensure_report_for_github(
        cosmos, project="ambience",
        repo="nelsong6/ambience", number=42,
...
def _comment(gh_id: int | None, body: str = "looks good") -> ReportComment:
...
async def test_append_pr_comment_appends(cosmos):
    pr = await create_report(cosmos, project="a", repo="r/n", number=1, title="t", branch="b")
    fetched, etag = await read_report(cosmos, project="a", report_id=pr.id)
    updated, _ = await append_report_comment(
...
async def test_append_pr_comment_dedupes_on_gh_id(cosmos):
    ...
    pr = await create_report(cosmos, project="a", repo="r/n", number=1, title="t", branch="b")
    fetched, etag = await read_report(cosmos, project="a", report_id=pr.id)
    once, etag1 = await append_report_comment(
        cosmos, pr=fetched, etag=etag, comment=_comment(gh_id=100, body="first"),
    )
    twice, _ = await append_report_comment(
...
def _review(gh_id: int | None, state: ReportReviewState = ReportReviewState.APPROVED) -> ReportReview:
    return ReportReview(
        id=str(ULID()),
        gh_id=gh_id,
        author="bob",
...
async def test_append_pr_review_appends(cosmos):
    pr = await create_report(cosmos, project="a", repo="r/n", number=1, title="t", branch="b")
    fetched, etag = await read_report(cosmos, project="a", report_id=pr.id)
    updated, _ = await append_report_review(
        cosmos, pr=fetched, etag=etag, review=_review(gh_id=200),
    )
    assert len(updated.reviews) == 1
    assert updated.reviews[0].state == ReportReviewState.APPROVED
...
async def test_append_pr_review_dedupes_on_gh_id(cosmos):
    pr = await create_report(cosmos, project="a", repo="r/n", number=1, title="t", branch="b")
    fetched, etag = await read_report(cosmos, project="a", report_id=pr.id)
    once, etag1 = await append_report_review(
        cosmos, pr=fetched, etag=etag, review=_review(gh_id=200, state=ReportReviewState.APPROVED),
    )
    twice, _ = await append_report_review(
        cosmos, pr=once, etag=etag1, review=_review(gh_id=200, state=ReportReviewState.CHANGES_REQUESTED),
    )
    assert len(twice.reviews) == 1
    assert twice.reviews[0].state == ReportReviewState.APPROVED


# ─── github_pull_request_url_for ───────────────────────────────────────


def test_github_pull_request_url_for_renders_canonical_form():
    assert github_pull_request_url_for("nelsong6/ambience", 42) == "https://github.com/nelsong6/ambience/pull/42"
